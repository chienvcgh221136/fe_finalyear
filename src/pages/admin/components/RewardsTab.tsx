...
import { Plus, Edit2, Trash2, Package, Tag, Layers, Check, X } from 'lucide-react';
...
const AdminRewards = () => {
    ...
    const createMutation = useMutation({
        ...
        onSuccess: () => {
            success("Tạo gói quà thành công!");
            setIsModalOpen(false);
            setEditingReward(null);
            queryClient.invalidateQueries({ queryKey: ['admin-rewards'] });
        },
        onError: (err: any) => error(err.response?.data?.message || "Lỗi tạo gói")
    });
    ...
    const updateMutation = useMutation({
        ...
        onError: (err: any) => error(err.response?.data?.message || "Lỗi cập nhật")
    });
    ...
    const deleteMutation = useMutation({
        ...
        onError: (err: any) => error(err.response?.data?.message || "Lỗi xóa")
    });
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h2 className="text-xl font-bold text-gray-900">Danh sách Gói quà</h2>
                    <p className="text-gray-500">Cấu hình các gói đổi thưởng cho người dùng</p>
                </div>
                    ...
                    {data?.map((reward: any) => (
                        <div key={reward._id} className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm hover:shadow-md transition relative group">
                            ...
                            <div className="flex items-end justify-between border-t border-gray-50 pt-4">
                                <div>
                                    <p className="text-xs text-gray-400 font-medium uppercase tracking-wider">Chi phí</p>
                                    <p className="font-bold text-indigo-600 text-xl">{reward.points.toLocaleString()} điểm</p>
                                </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden animate-fade-in max-h-[90vh] overflow-y-auto">
                <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                    <h3 className="text-xl font-bold text-gray-900">{initialData ? 'Chỉnh sửa Gói' : 'Tạo Gói Mới'}</h3>
                ...
                <form onSubmit={(e) => onSubmit(e, formData)} className="p-6 space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option value="VIP">VIP (Gói VIP)</option>
                                <option value="CREDIT">CREDIT (Lượt xem)</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div className="pt-4 flex justify-end gap-3">
                        <Button variant="ghost" type="button" onClick={onClose} disabled={isLoading}>Hủy</Button>
